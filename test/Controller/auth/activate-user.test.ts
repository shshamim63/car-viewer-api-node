...
describe('activate/user', () => {
    ...
    test('It should throw error when trying to activate the same user', async () => {
        const userHelperMock = jest.spyOn(userHelper,'findOneUser').mockResolvedValue({...userData, status: ZodActiveStatusEnum.Enum.Pending })
        const findAndUpdateUserByIdMock= jest.spyOn(userHelper,'findAndUpdateUserById').mockResolvedValue({...userData, status: ZodActiveStatusEnum.Enum.Pending })
        expect(userHelperMock).toHaveBeenCalled()
        expect(findAndUpdateUserByIdMock).toHaveBeenCalled()
        const token = generateToken(userData, authConfig.accessTokenSecret)
        const response = await request(app).get(`/activate/user?token=${token}`)
